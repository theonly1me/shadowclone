...
The wizard derives its questions from `SeedSkillLibrary`. It makes eight decisions: one numbered choice for each of the six axes, one numbered multi-select for all seven disciplines, and one confirmation after printing every selected title. The discipline response accepts `all`, `none`, or a comma-separated set of displayed numbers. There is no separate autonomy question because `question-frequency` already represents that choice.
...
Selected skills become global `ProfileRule` records with `source: "declared"`, `status: "active"`, zero evidence, and the skill's section and applicability. Their persistent key is `seed:<skill-id>`. The key is assigned by the registry identity rather than visible wording, so changing a title never creates a second declared rule.
...
`shadowclone wizard` runs only the skill selection and profile update. It does not repeat capture consent. `shadowclone init` skips the wizard when a rules file is present, runs it otherwise, and always performs consent last. Only present transcript sources receive named questions. Git metadata, eval context, and deep distillation remain separate capability questions because they are not transcript provider roots.
...
The wizard reads package-owned seed Markdown through the PR 5 loader. After explicit confirmation, it writes selected titles, bodies, applicability, and fixed registry ids to the user's local profile through `writeProfile`. This is declared guidance chosen by the user, not captured machine content, so `resolveRedacted` is not part of the path.
...
**Install a recommended skill bundle.** This would label maintainer preferences as the user's declared behavior. Every axis requires an explicit choice and every discipline is opt-in.

**Ask one yes-or-no question per discipline.** Thirteen skill questions plus confirmation would be repetitive. One numbered multi-select keeps all disciplines explicit while holding the profile portion to eight decisions.
...
2026-09-08: Ask eight profile decisions before capture consent because the user should understand the result before deciding what local data may produce evidence.
...
2026-09-08: Derive every skill choice from the packaged registry because a second hardcoded question-to-rule mapping would drift.

2026-09-08: Use stable `seed:<skill-id>` profile keys because registry identity must survive wording changes and enable later axis reconciliation.
