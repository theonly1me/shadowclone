...
**The loop.** `src/observe/` reads enabled transcript sources as text pointers. `src/index/` stores pointers and event skeletons, never captured text. `src/redact/` is the only place a pointer becomes redacted text. Signal derivation, profiles, engines, and acting are not built yet.
