...
3. **A test proves the wiring, not just the function.** `src/redact/index.test.ts` proving a pattern works says nothing about whether the collector calls it. Add the `src/collector.test.ts` kind, and prove it catches the bug by mutating the call away and watching it go red. Every adapter under `src/observe/adapters/` ships one, with a fixture transcript holding a planted secret.
